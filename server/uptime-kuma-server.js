...
const { canViewAllMonitors } = require("./socket-permissions");
...
class UptimeKumaServer {
     ...
     */
    constructor() {
        // Set axios default user-agent to Ayoratou/version
    ...
    async sendMonitorList(socket) {
        ...
        // Addressed to this socket rather than the owner's room: two users in the
        // same room can have different visibility, so a room-wide broadcast would
        // hand one user's list to the other.
    ...
    async sendUpdateMonitorIntoList(socket, monitorID) {
        ...
        if (list && list[monitorID]) {
            socket.emit("updateMonitorIntoList", list);
        }
    }
    ...
    async getMonitorJSONList(userID, monitorID = null) {
        ...
        // Users holding monitor.view.all see every monitor, not just their own.
        let query = canViewAllMonitors(user) ? " 1 = 1 " : " user_id = ? ";
        let queryParams = canViewAllMonitors(user) ? [] : [userID];
     ...
     */
    getUserAgent() {
        return "Ayoratou/" + require("../package.json").version;
    }
