            ...
            if (bean.status !== MAINTENANCE && Boolean(this.domainExpiryNotification)) {
                try {
                    ...
                    if (
                        ...
                        log.warn(
                            "domain_expiry",
                            `Domain expiry unsupported for '.${error.meta.publicSuffix}' because it lacks an RDAP endpoint in the IANA database. This isn’t an Ayoratou bug, a limitation of your registry. If an RDAP server exists, ask your registrar politely to submit it to IANA so expiry checks can work.`
                        );
                    }
                }
            }
